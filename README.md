...
- [Web Authentication, Web Credentials and Time based one time passwords (TOTP) with JWTs (JSON Web Tokens)](authenticate.md) instead of traditional username/passwords to perform multiple factor authentication on user requests and apply the considered user permissions and entitlements for the request or entitlement.
- [REST APIs](api.md) to make requests and receive immediate responses.
- [Server Sent Events](sse.md) (SSE) and
